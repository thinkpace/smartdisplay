...
func get_animations() -> PackedStringArray:
	Logger.debug("unicorn.get_animations() -> PackedStringArray")
...
func animate(animation: String):
	Logger.debug("unicorn.animate(" + animation + ": String)")
	...
	var source_x = 0
	@warning_ignore("integer_division")
	var source_y = int(Config.get_viewport_size().y/8 + size.y/2 + random_offset_y)
	var target_x = 0
	@warning_ignore("integer_division")
	var target_y = int(Config.get_viewport_size().y/8 + size.y/2 + random_offset_y)
...
func _on_tween_finished():
	Logger.debug("unicorn._on_tween_finished()")
